    ...
    def __init__(self, parent, controller):
        ...
        def second_check():
            ...
            if(RadioVariety_1.get() == 0):
                ...
                panel.image = img
                panel.place(x=620,y=50)
                printData()
                
                def close_image():  #사진 닫기
                    panel.destroy() 

                '''
                close_radio=tk.Radiobutton(self, text="X", command=close_image)
                close_radio.place(x=1200,y=20)'''
                close_radio = tk.Button(self, text=" X ", command=close_image)
                close_radio.place(x=1250,y=20)
        ...
        label2 = tk.Label(self, text="제목",font=('Arial', 20), background=controller.backcolor)
        ...
        #클라우드 cloud 이미지 image
        CloudPhoto = tk.PhotoImage(file="clouds.png")
        clouds = tk.Label(self, image = CloudPhoto, background=controller.backcolor)
        clouds.image = CloudPhoto
        clouds.place(x=850,y=470)
...
class PageTwo(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        Homebutton = tk.Button(self, image = HomePhoto, background=controller.backcolor, command=lambda: controller.show_frame("StartPage"))
        Homebutton.image = HomePhoto
        Homebutton.place(x=1150,y=550)


        #label.place(x=0,y=0)
        #Backbutton.place(x=1100,y=620)
        ...
        ##db 텍스트 상자
        textbox2 = tk.Text(self, width=50, height=20,font=("System", 15))   ###
        #textbox2.resizable(width= tk.FALSE, height= tk.FALSE)

        textbox2.place(x=60,y=0)
        ...
        RefreshPhoto = tk.PhotoImage(file="RefreshButton.png")
        Refreshbutton = tk.Button(self, image = RefreshPhoto, background=controller.backcolor, command=db_text,)
        Refreshbutton.image = RefreshPhoto
        Refreshbutton.place(x=60,y=550)

        #구름사진
        CloudPhoto = tk.PhotoImage(file="clouds2.png")
        clouds = tk.Label(self, image = CloudPhoto, background=controller.backcolor)
        clouds.image = CloudPhoto
        clouds.place(x=700,y=100)
